...
export default function DailyForecast({ data, isLoading }: DailyForecastProps) {
  ...
  return (
    <section className="bg-card text-card-foreground py-6 px-4 shadow-sm mb-4 border-b border-border">
      <div className="container mx-auto">
        <h3 className="text-lg font-medium text-foreground mb-4">5-Day Forecast</h3>
        ...
        <div className="space-y-4">
          {data.map((day, index) => {
            ...
            return (
              <div 
                key={index} 
                className="flex items-center justify-between py-2 hover:bg-accent rounded-md px-2 cursor-pointer transition-colors"
              >
                <div className="flex-1">
                  <div className="text-sm font-medium text-foreground">{formatDay(day.dt)}</div>
                </div>
                ...
                <div className="flex-1 flex items-center justify-end">
                  <div className="w-full max-w-[100px]">
                    <div className="flex items-center">
                      <span className="text-xs text-muted-foreground w-6">{precipProbability}%</span>
                      <div className="flex-1 mx-2">
                        <div className="h-1.5 bg-muted rounded-full">
                          <div 
                            className="h-1.5 bg-weather-accent rounded-full" 
                  ...
                  <div className="text-sm font-medium text-muted-foreground ml-2 w-8 text-right">
                    {formatTemp(day.temp.min, settings.temperatureUnit)}
                  </div>
                  <div className="w-16 mx-2">
                    <div className="h-1 bg-muted rounded-full relative">
                      <div 
                        className="h-1 bg-foreground/55 dark:bg-weather-accent rounded-full absolute inset-0" 
                        style={{ 
