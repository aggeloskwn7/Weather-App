...
export default function HourlyForecast({ data, sunrise, sunset, isLoading }: HourlyForecastProps) {
  ...
  return (
    <section className="bg-card text-card-foreground py-6 px-4 shadow-sm mb-4 border-b border-border">
      <div className="container mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-foreground">Hourly Forecast</h3>
          <Button
            variant="ghost"
            size="sm"
            onClick={toggleHourlyView}
            className="text-weather-accent text-sm font-medium hover:text-weather-accent hover:bg-weather-accent-muted"
          >
          ...
          {hoursToShow.map((hour, index) => {
            ...
            return (
              <div key={index} className="flex-shrink-0 text-center px-3 py-2 w-20">
                ...
                <div className="text-xs text-muted-foreground mt-1">{Math.round(hour.pop * 100)}%</div>
              </div>
            );
          })}
        </div>
